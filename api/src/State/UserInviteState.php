...
{
    public function __construct(private EventRepository $eRepo, private MailerInterface $mailer) {}
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): void
    {
        // $data is the userInvite object
        $data->setOrganization(
            $this->eRepo->getOrganizationById($uriVariables['eventId'])
        );
        ...
        foreach ($data->getEmails() as $email) {
            $inviteLink = $inviteLinkBase . '?eventCode=' . $data->getOrganizationInviteCode() . '&email=' . $email;
            $email = (new Email())
                ->to($email)
